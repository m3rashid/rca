...
export interface IRegistration extends BaseModel {
  user?: IUser; // required: ObjectId
  completedSteps: Array<number>;
  currentStep: number;
  gender: string; // required:
  fatherName: string; // required:
  motherName: string; // required:
  dateOfBirth: Date; // required: // also calculate age from this
  mobileNumber: string; // required:
  phoneNumber?: string;
  permanentAddress?: IAddress; // required:
  correspondenceAddress?: IAddress; // required:
  education?: Array<IEducation>; // required:
  testCenter?: ITestCenter; // required: // ObjectId
  transactionId?: string; // required:
  earlierCompetitiveExams: Array<IEarlierCompetitiveExams>; // required:
  // required:
  agreeToTerms?: {
    informationIsCorrect: boolean; // required:
    rightToChange: boolean; // required:
  };
  rollNumber?: string; // generated by backend
  category?: string; // required: // General, SC/ST, OBC, PWD, Gen-EWS, EWS
